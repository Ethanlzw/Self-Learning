...
$\quad \cdot$ 物理意义：当你在节点 1 ($\xi = -1$) 时，$N_1 = 1$, $N_2 = 0$；当你在节点 2 ($\xi = 1$) 时，$N_1 = 0$, $N_2 = 1$。这满足了“单位分解 (Partition of Unity)”的要求，即 $N_1 + N_2 = 1$。  
...
因为我们现在有两个坐标系（全局坐标 $x$ 和自然坐标 $\xi$），我们需要一个转换系数，这就是雅可比矩阵 (Jacobian)。对于一维问题，它就是一个标量，表示局部坐标对全局坐标的缩放比例：  
...
$$
J = \frac{d}{d\xi} \left( \frac{1-\xi}{2} x_1 + \frac{1+\xi}{2} x_2 \right) = -\frac{x_1}{2} + \frac{x_2}{2} = \frac{x_2 - x_1}{2} = \frac{L}{2}
$$  

$\quad \cdot$ 结论：对于一维线性单元，雅可比 $J = \frac{L}{2}$。  

4. 求解应变与 B 矩阵 (Strain and B-Matrix)  
根据固体力学，应变 $\epsilon$ 是位移 $u$ 对物理坐标 $x$ 的导数。但我们的 $u$ 现在是用 $\xi$ 表达的，所以需要用链式法则：  

$$
\epsilon = \frac{du}{dx} = \frac{du}{d\xi} \cdot \frac{d\xi}{dx}
$$

$\quad$ 因为 $J = \frac{dx}{d\xi}$，所以 $\frac{d\xi}{dx} = \frac{1}{J} = \frac{2}{L}$.  
$\quad$ 现在我们对位移公式 $u = N_1 u_1 + N_2 u_2$ 关于 $\xi$ 求导：  
...
$\quad$ 对于一维横截面积为 $A$ 的杆件，$dV = A dx$。并且我们把积分变量从 $dx$ 换成 $d\xi$，需要乘上雅可比 $J$：  

$$
dx = J d\xi = \frac{L}{2} d\xi
$$

$\quad$ 积分范围也从物理长度变为了[-1, 1]：  

$$
K^e = \int_{-1}^{1} \begin{bmatrix} -\frac{1}{L} \cr \frac{1}{L} \end{bmatrix} E \begin{bmatrix} -\frac{1}{L} & \frac{1}{L} \end{bmatrix} A \left( \frac{L}{2} \right) d\xi
$$

$\quad$ 由于积分里的各项 $E$, $A$, $L$ 在这个简单的线性单元中都是常数，我们可以直接把它们提取到积分号外面：  

$$
K^e = E A \left( \frac{L}{2} \right) \begin{bmatrix} \frac{1}{L^2} & -\frac{1}{L^2} \cr -\frac{1}{L^2} & \frac{1}{L^2} \end{bmatrix} \int_{-1}^{1} d\xi
$$

$\quad$ 由于 $\int_{-1}^{1} d\xi = 2$：  
