...
$\quad F$: 力（Load）  
$\quad K$: 刚度（Stiffness，代表物体抵抗变形的能力）  
$\quad d$: 位移（Displacement）  
...
$$  
$\qquad K^e$：单元刚度矩阵（这是我们最终要计算的目标）  
$\qquad B$：应变-位移矩阵（描述节点位移如何产生内部应变）  
$\qquad D$：本构矩阵（材料的弹性性质矩阵，包括杨氏模量等）  
$\qquad V$：单元的体积  
...
$\quad *$ 无论实际杆件在全局坐标系 $x$ 中的长度 $L$ 是多少，在自然坐标系中，它的起点永远是 $\xi = -1$，终点永远是 $\xi = 1$。  
$\quad *$ 等参概念 (Iso-parametric) 的意思是：我们用同一套函数来同时插值“几何坐标”和“位移”。
